...
**Updated 2026-09-21: approximately 59% engineering completion; no accepted
...
voice passes/provider compatibility and granular controls add **3.6 points**, bringing current completion to **59.1**.
...
removes its previously earned **0.50 overall points**; valid historical scoring
evidence remains. NS-3 returns to **30%**, and overall completion to **59.1%**.
The repair retains the original task allocation and creates no additional credit.
...
| <a id="wav-02"></a><a id="wav-03"></a><a id="ns-3"></a>**NS-3 — Trustworthy musical learning from WAV** | **30%** | 25 | Source-bound measurements, selected clocks, uncertainty/manual context, narrow pitch/duration learning and [shared scoring with reopened admission](EVALUATION-OPERATOR.md). [Phrase](PHRASE-EVALUATION.md) and [pulse](BEAT-TRACKING.md) evidence retain the accuracy gaps. | Independently validated context, notes, mixed parts, harmony, groove and evolving sound: **17 tasks across 10 WAV outcomes**. | **17.5** |
| <a id="wfc-preferences"></a><a id="ns-4"></a>**NS-4 — Granular WFC layers and reusable blends** | **84%** | 15 | Actual dependent passes, locks/preferences, owned timing, weighted saved blends/lineage, [named voice controls](INDEPENDENT-VOICES.md#named-independent-voice-api) and accepted [semantic provider compatibility/replacement](INDEPENDENT-VOICES.md#provider-compatibility-and-replacement) and [granular musical/sound controls](INDEPENDENT-VOICES.md#granular-musical-and-sound-controls). | Changing-duration edits, saved joint providers and audible generation/reblend: **WFC-LAYERS, WFC-STYLE, WAV-04-INTEGRATION**. | **2.4** |
...
| **Total** | **≈59%** | **100** | **59.1 weighted points credited** | **43 open tasks / 22 outcomes** | **40.9** |

Arithmetic: `10×1.00 + 25×0.80 + 25×0.30 + 15×0.84 + 20×0.27 + 5×0.72 = 59.1`.
...
**Musical learning and corpus quality account for 32.1 of the remaining 40.9
points (about 78%).** Additional diagnostics and mechanical adapters have value
...
**43 open task files own the remaining work across 22 outcomes; 6 tasks are DONE.**
...
and follows prerequisites. [Shared validation](TODO/NS-3_validation_01.md)
is reopened for missing prediction-ancestry checks; repair this prerequisite before accepting [style cards and comparators](TODO/NS-5_evaluation_01.md),
...
and [granular controls](TODO/DONE/NS-4_layers_03.md) are also accepted; that separate path can next address changing-duration edits and saved semantic providers.

The [working style specification](STYLE-CARDS.md) now defines provider-specific
comparators and source-bound acoustic controls. Musical reference annotations,
genre assignments and calibrated numerical criteria are still missing. This
...
| <a id="wav-validation"></a>**WAV-VALIDATION** | NS-3 | [shared evaluation — reopened](TODO/NS-3_validation_01.md), [NS-3_validation_02](TODO/NS-3_validation_02.md) |
...
| <a id="wfc-layers"></a>**WFC-LAYERS** | NS-4 | [NS-4_layers_04](TODO/NS-4_layers_04.md); accepted prerequisites: [named voices — DONE](TODO/DONE/NS-4_layers_01.md), [provider compatibility — DONE](TODO/DONE/NS-4_layers_02.md), [granular controls — DONE](TODO/DONE/NS-4_layers_03.md) |
...
completion from 59.1% to 60.1%, assuming no other changes. This makes bounded
outcome progress visible without waiting for all WAV learning to finish.
The remaining open task credits add **40.9 overall points**, reaching 100 only
when the required work and final scope audit are accepted.

## Execution order and blocking links

1. Repair shared prediction-ancestry admission; resolve pitch identity and
...
| --- | --- | ---: |
| **A — Dependable sound and trustworthy recorded musical inputs** | All NS-2 quality tasks (+5); NS-3 validation, tempo, context and notes tasks (+9.5). Actual listening and independent native note/context admission. | **+14.5 remaining** |
| **B — Recorded parts through reusable semantic generation** | NS-3 parts, harmony, groove and timbre (+8); remaining NS-4 duration edits, persistence, blend/reblend and audible integration (+2.4). | **+10.4 remaining** |
...
**59.1 current + 14.5 + 10.4 + 16.0 = 100.**
...
start with NS-5 and follow its actual prerequisite links. Reopened shared
[admission/scoring](TODO/NS-3_validation_01.md) must be repaired before accepting the
...
baseline. Named voices, compatibility and granular controls are accepted; the independent semantic-layer
path can continue with [changing-duration edits](TODO/NS-4_layers_04.md) and [saved semantic providers](TODO/NS-4_styles_01.md).
