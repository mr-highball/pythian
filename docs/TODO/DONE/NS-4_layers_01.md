...
Completed 2026-09-21: the
[named independent-voice API](../../INDEPENDENT-VOICES.md#named-independent-voice-api)
owns exact companion models, typed role choices, staged named masks and actual
dependency-scoped regeneration, retaining unrelated pending edits and accepted
states. It preserves collective witness passes, ranges, pair constraints and the
eight musical-layer bound. The previously unvalidated provider variants now feed
this consumer; the demo uses its reusable planning/capture and voice-choice query.
An independent maintained caller covers one-role edits and failure/ownership
boundaries. Final checked stable FPC 3.2.2 Win32/Win64 fixtures pass, with no
unfreed blocks. Three demo scenarios per target (authored, saved cell-style and
changing-tempo duration) reproduce all 24 target-matched WAV/preview/MIDI/JSON
artifacts against frozen source `0ecfe34`. Commands, input identities and logs are
retained under `build/semantic-voices/`; the final six-criterion audit is under
`build/qa-batch-02/`. The maintained build includes the independent caller.
No role inference, listening or genre acceptance is claimed. Accepted +6 NS-4
points (+0.90 overall), moving NS-4 from 70% to 76%. Together with the accepted
shared validation task (+0.50), overall completion moves from 57.0% to 58.4%.
